...
评测必须模拟下面这个真实流程，而不是让模型自行决定标注词：
1. 先在本地做预处理：
   - 对句子逐词处理，使用 wink-lemmatizer 做词形还原；
   - 英文缩写词（如 we'll、she's、I'm、don't、p.m.）直接跳过，不加中括号，不进入 targetWords；
   - 冠词特殊归并：an 统一按 a 处理（避免把 an 当成未收录词）；
   - 人名过滤：默认启用 `person_only`，疑似人名（如 Connell / Marianne / Lorraine）不加中括号，不进入 targetWords；
   - 专有名词过滤模式支持代码级配置：
...
2. 再把 annotatedText + targetWords 发给大模型：
   - 模型只能翻译 targetWords 中的词；
   - 模型不能新增、删除、替换标注词；
   - sentence 必须与 annotatedText 逐字符一致；
   - meaning 必须是对象，键集合与 targetWords 完全一致（小写键）。
3. 前端渲染时中括号不展示，只做可点击下划线；点击后根据单词 key 命中 meaning。

模型效果评分重点：
1. JSON 合法性与可解析性；
2. sentence 是否严格等于 annotatedText；
3. meaning 键是否与 targetWords 一致（无缺失、无多余）；
4. 中文释义是否自然且符合句子语境；
5. 速度（响应时延）；
6. 综合表现（又快又稳又聪明）。
