...
- 80 端口通过 Nginx 反代到 `127.0.0.1:3000`。
- 当 `pm2` 停止 Nuxt 进程后，访问 `/` 无法打开备案页。

### 根因

1. Nginx 原配置是全路径反向代理到 Node 服务。
2. 备案页虽然是静态文件，但入口依赖 Node 进程返回，进程停掉后不可用。

### 解决方案

- `scripts/deploy-aliyun.sh`
  - `--nginx-80-proxy` 模式下生成新 Nginx 配置：
    - `location = /` 与 `location = /beian-love-record.html` 直接从 `${REMOTE_PATH}/.output/public` 读取静态文件；
    - 其余路径继续反代到 `127.0.0.1:3000`；
    - 增加 `502/503/504` 错误回退到备案页。
  - 增加静态页存在性检查，避免写入无效配置。

### 验证

- 执行部署命令后，即使停止 `pm2` 进程，访问 `http://<host>/` 仍可打开 `beian-love-record.html`。
